...
def solve_ground_state(Hs: np.ndarray, iters: int, pow_k: int):
    ...
    # convergence diagnostics on original Hs
    HsPhi = Hs @ Phi
    lam = float(Phi @ HsPhi)  # Rayleigh quotient (||Phi||=1)
    r = HsPhi - lam * Phi
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Local Grey25+Z prediction with frozen driver and neighborhood averaging"
    )
